...
void callback(heron_controller::controlConfig& config, uint32_t level){
	...
	rotation_ratio = config.rotation_ratio;
	ROS_INFO("%f,%lf,%lf,%lf,%lf,%lf",width_robot,backward_ratio,forward_ratio,backward_offset,forward_offset,rotation_ratio);
}
...
float yawrate_to_speed(float th){
	// th = (right - left) / d where d is the width of heron
	float speed = th * width_robot*rotation_ratio;
	return speed;
...
void cmd_velCallback(const geometry_msgs::Twist& twist_aux){
	...
	float left_vel = 0.0;
	float speed;
 	speed = yawrate_to_speed(vel_th);
	if (speed/2 >  backward_ratio){
		right_vel = vel_x + speed / 2;
		left_vel = vel_x - backward_ratio;
	}
	else if (speed/2 < (-1)*backward_ratio){
		right_vel = vel_x - backward_ratio;
		left_vel = vel_x - speed/2;
	...
	/*if(vel_th == 0){
		...
		right_vel = rotation_ratio * vel_th * width_robot;
		left_vel = (-1)* right_vel;
		
	}
	else{
		//Linear and angular speed are both not zero
		right_vel = vel_x + vel_th * width_robot * rotation_ratio;
		left_vel = vel_x - vel_th * width_robot * rotation_ratio;
	}
	*/
	
	vr = constraint(right_vel);
	ROS_INFO("The value for right_vel: %f", right_vel);
	vl = constraint(left_vel);
